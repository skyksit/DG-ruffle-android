...
/// Convert an Android keycode integer to Keycode enum
pub fn keycode_from_int(keycode: i32) -> Option<Keycode> {
    // This uses the from_keycode method from android_activity
    Keycode::from_keycode(keycode as u32)
}
...
pub fn keycode_to_key_descriptor(keycode: i32) -> Option<KeyDescriptor> {
    let android_keycode = keycode_from_int(keycode)?;
    
    // Reuse the logic from android_key_event_to_ruffle_key_descriptor
    let physical_key = PhysicalKey::Unknown;

    let logical_key = match android_keycode {
        Keycode::DpadUp => LogicalKey::Named(NamedKey::ArrowUp),
        Keycode::DpadDown => LogicalKey::Named(NamedKey::ArrowDown),
        Keycode::DpadLeft => LogicalKey::Named(NamedKey::ArrowLeft),
        Keycode::DpadRight => LogicalKey::Named(NamedKey::ArrowRight),
    ...
    let key_location = match android_keycode {
        Keycode::AltLeft | Keycode::ShiftLeft | Keycode::CtrlLeft | Keycode::MetaLeft => {
            KeyLocation::Left
        }
